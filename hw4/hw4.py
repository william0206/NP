...
sub_status = {} #for subscribing board
sub_author = {} #for subscribing author

def new_sub_status(s, sokt):
    global sub_status
    sub_status[status[sokt]] = s

def new_sub_author(s, sokt):
    global sub_author
    sub_author[status[sokt]] = s
        ...
        if skt is server_socket:
            ...
            if msg is not "": #not "" means it's not sending nothing
                ...
                if cmd[0] == 'exit':
                    if len(cmd) == 1:
                        if status[skt] != '0':
                            sub_status.pop(status[skt])
                            sub_author.pop(status[skt])
                    ...
                    if check_for_post != None:
                        if status[skt] != '0':
                            ...
                            if not data:
                                ...
                                #handle subscribtion activity
                                # for s in inputs:
                                #     if s is not server_socket:
                                for s in sub_status:
                                    for o in sub_status[s]:
                                        if o[0] == post[1] and (re.search(str(o[1]), post[2]) != None):    
                                            for u in inputs:
                                                if u is not server_socket and status[u] == s:
                                                    u.send('*[{}] {} - by {}*\n% '.format(post[1], post[2], status[skt]).encode('utf-8'))

                                for s in sub_author:
                                    for o in sub_author[s]:
                                        if o[0] == status[skt] and (re.search(str(o[1]), post[2]) != None):    
                                            for u in inputs:
                                                if u is not server_socket and status[u] == s:
                                                    u.send('*[{}] {} - by {}*\n% '.format(post[1], post[2], status[skt]).encode('utf-8'))
                        else:
                    ...
                    if len(cmd) == 5:
                        if status[skt] != '0':
                            if cmd[1] == '--board':
                                check_for_sub = 0
                                for sub in sub_status[status[skt]]:
                                    if sub[0] == cmd[2] and sub[1] == cmd[4]:
                                        skt.send('Already subscribed\n'.encode('utf-8'))
                                        check_for_sub = 1
                                        break
                                if check_for_sub == 0:
                                    sub_status[status[skt]].append((cmd[2], cmd[4]))
                                    skt.send('Subscribe successfully\n'.encode('utf-8'))

                            elif cmd[1] == '--author':
                                check_for_sub = 0
                                for sub in sub_author[status[skt]]:
                                    if sub[0] == cmd[2] and sub[1] == cmd[4]:
                                        skt.send('Already subscribed\n'.encode('utf-8'))
                                        check_for_sub = 1
                                        break
                                if check_for_sub == 0:
                                    sub_author[status[skt]].append((cmd[2], cmd[4]))
                                    skt.send('Subscribe successfully\n'.encode('utf-8'))
                            else:
                                skt.send('subscribe --board/author <boardname>/<author name> --keyword <keyword>\n'.encode('utf-8'))
                        else:
                            skt.send('Please login first.\n'.encode('utf-8'))
                    else:
                        if cmd[1] == '--board':
                            skt.send('Usage: subscribe --board <board-name> --keyword <keyword>\n'.encode('utf-8'))
                        elif cmd[1] == '--author':
                            skt.send('Usage: subscribe --author <author-name> --keyword <keyword>\n'.encode('utf-8'))
                        else:
                            skt.send('Usage: subscribe --board/author <board-name>/<author-name> --keyword <keyword>\n'.encode('utf-8'))
                elif cmd[0] == 'unsubscribe':
                    if len(cmd) == 3:
                        if status[skt] != '0':
                            if cmd[1] == '--board':
                                l = len(sub_status[status[skt]])
                                check_for_unsub = 0
                                temlist = []
                                for s in sub_status[status[skt]]:
                                    if s[0] != cmd[2]:
                                        check_for_unsub = check_for_unsub + 1
                                        temlist.append(s)
                                new_sub_status(temlist, skt)
                                if check_for_unsub == l:
                                    skt.send('You haven\'t subscribed {}\n'.format(cmd[2]).encode('utf-8'))
                                else:
                                    skt.send('Unsubscribe successfully\n'.encode('utf-8'))
                            elif cmd[1] == '--author':
                                l = len(sub_author[status[skt]])
                                check_for_unsub = 0
                                temlist = []
                                for s in sub_author[status[skt]]:
                                    if s[0] != cmd[2]:
                                        check_for_unsub = check_for_unsub + 1
                                        temlist.append(s)
                                new_sub_author(temlist, skt)
                                if check_for_unsub == l:
                                    skt.send('You haven\'t subscribed {}\n'.format(cmd[2]).encode('utf-8'))
                                else:
                                    skt.send('Unsubscribe successfully\n'.encode('utf-8'))
                            else:
                                skt.send('Usage: unsubscribe --board/author <board-name>/<author-name>\n'.encode('utf-8'))
                        else:
                            skt.send('Please login first.\n'.encode('utf-8'))
                    else:
                        if cmd[1] == '--board':
                            skt.send('Usage: unsubscribe --board <board-name>\n'.encode('utf-8'))
                        elif cmd[1] == '--author':
                            skt.send('Usage: unsubscribe --author <author-name>\n'.encode('utf-8'))
                        else:
                            skt.send('Usage: unsubscribe --board/author <board-name>/<author-name>\n'.encode('utf-8'))
                elif cmd[0] == 'list-sub':
                    if status[skt] != '0':
                        skt.send("Board :\n".encode('utf-8'))
                        if sub_status[status[skt]]:
                            for s in sub_status[status[skt]]:
                                skt.send("{}: {}\n".format(s[0], s[1]).encode('utf-8'))
                        # skt.send("\n".encode('utf-8'))
                        skt.send("-------\nAuthor:\n".encode('utf-8'))
                        if sub_author[status[skt]]:
                            for s in sub_author[status[skt]]:
                                skt.send("{}: {}\n".format(s[0], s[1]).encode('utf-8'))
                    else:
                        skt.send('Please login first.\n'.encode('utf-8'))
