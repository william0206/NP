                ...
                if cmd[0] == 'exit':
                    ...
                    if check_for_post != None:
                        if status[skt] != '0':
                            ...
                            #check_for_board = 0
                            data = c.execute('SELECT Name FROM BOARD WHERE Name = ?', (post[1],)).fetchone()
                            if not data:
                                ...
                                post_id = c.execute('SELECT * FROM POST WHERE ID = (SELECT MAX(ID) FROM POST)').fetchone()[0]
                                skt.send(str('Create post successfully.post_id:' + str(post_id) +'\n').encode('utf-8'))
                        else:
                            skt.send('Please login first.\n'.encode('utf-8'))
                    else:
                    ...
                    # if status[skt] != '0':
                        if len(cmd) == 1:
                            ...
                            board_index = 1
                            for r in row:
                                skt.send('   {:<12}{:<14}{:<14}\n'.format(str(board_index), r[1], r[2]).encode('utf-8'))
                    ...
                    if len(cmd) == 2:
                        # if status[skt] != '0':
                            ...
                            if not data:
                                ...
                                skt.send('   Author   :{}\n   Title    :{}\n   Date     :{}\n   --{}\n'.format(data[2], data[1], data[3], data[2]).encode('utf-8'))
                    ...
                    check_format = re.fullmatch(r'update-post .+ --title .+|update-post [0-9]+ --content .+', msg) 
                            ...
                            if re.search('title', msg) != None:
                                ...
                                if not post_data:
                                    ...
                                    if post_data[2] == status[skt]:
                                        ...
                                        # author = c.execute('SELECT * FROM POST WHERE ID = ?', (int(u_post[1]), )).fetchone()[2]
                                        # skt.send(str('Update successfully.' + 'author:' + '{}'.format(author)).encode('utf-8'))
                                        skt.send('Update successfully.\n'.encode('utf-8'))
                                    else:
                                        skt.send('Not the post owner.\n'.encode('utf-8'))
                        else:
                    ...
                    if check_format != None:
                        if status[skt] != '0':
                            cmt = msg.split(' ', 2)
                            post_data = c.execute('SELECT * FROM POST WHERE ID = ?', (cmt[1], )).fetchone()
                            if not post_data:
                                skt.send('Post does not exist.\n'.encode('utf-8'))#
                            else:
                                # c.execute('INSERT INTO COMMENT (post_id, user, comment) VALUES (?, ?, ?)', (int(cmt[1]),status[skt], cmt[2]))
                                skt.send(str('Comment successfully.' + 'author:' + '{}'.format(post_data[2])).encode('utf-8'))
                        else:
                            skt.send('Please login first.\n'.encode('utf-8'))#
                    else:
                    ...
                    check_for_mail = re.fullmatch(r'mail-to .+ --subject .+ --content .+', msg)
                    if check_for_mail != None:
                        if status[skt] != '0':
                            mail = re.split(r'mail-to | --subject | --content ', msg)
                            user = c.execute('SELECT username FROM USERS WHERE username = ?', (mail[1],)).fetchone()
                            if not user:
                                skt.send('{} does not exist.\n'.format(mail[1]).encode('utf-8'))
                            else: # username do exist
                                today = str(date.today())
                                c.execute('INSERT INTO MAIL (Subject, Sender, Receiver, Date) VALUES (?, ?, ?, ?)', (mail[2], status[skt], mail[1], today))
                                mail_id = c.execute('SELECT * FROM MAIL WHERE ID = (SELECT MAX(ID) FROM MAIL)').fetchone()[0]
                                
                                skt.send(str('Sent successfully.' + 'mail_id:{}'.format(str(mail_id)) + '\n').encode('utf-8'))

                        else:
                            skt.send('Please login first.\n'.encode('utf-8'))
                    else:
                        skt.send('Usage: mail-to <username> --subject <subject> --content <content>\n'.encode('utf-8'))
                elif cmd[0] == 'list-mail':
                    if status[skt] != '0':
                        if len(cmd) == 1:
                            skt.send('   ID       Subject             From         Date   \n'.encode('utf-8'))
                            mail_metadata = c.execute('SELECT * FROM MAIL WHERE Receiver = ?', (status[skt],)).fetchall()
                            count = 0
                            for m in mail_metadata:
                                count = count + 1
                                skt.send('   {:<9}{:<20}{:<13}{:<7}\n'.format(str(count), m[1], m[2], m[4].split('-')[1] + '/' + m[4].split('-')[2]).encode('utf-8'))
                        else:
                            skt.send('Usage: list-mail\n'.encode('utf-8')) 
                    else:
                        skt.send('Please login first.\n'.encode('utf-8'))
                elif cmd[0] == 'retr-mail' or cmd[0] == 'delete-mail':
                    if status[skt] != '0':
                        if len(cmd) == 2:
                            if cmd[0] == 'delete-mail':
                                mail_data = c.execute('SELECT * FROM MAIL WHERE Receiver = ?', (status[skt],)).fetchall()
                                mail_count = 0
                                for m in mail_data:
                                    mail_count = mail_count + 1
                                    if mail_count == int(cmd[1]):
                                        c.execute('DELETE FROM MAIL WHERE ID = ?', (m[0], ))
                                        break
                            skt.send('Success'.encode('utf-8'))
                        else:
                            if cmd[0] == 'retr-mail':
                                skt.send('Usage: retr-mail <mail#>\n'.encode('utf-8'))
                            else:
                                skt.send('Usage: delete-mail <mail#>\n'.encode('utf-8'))
                    else:
                        skt.send('Please login first.\n'.encode('utf-8'))
                else:
                    # print(msg)
